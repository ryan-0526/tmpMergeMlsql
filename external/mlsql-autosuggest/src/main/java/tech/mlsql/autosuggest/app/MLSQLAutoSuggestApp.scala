...
import tech.mlsql.autosuggest.utils.{PlugGatherInit, SparkFunctionInit}
...
class MLSQLAutoSuggestApp extends tech.mlsql.app.App with VersionCompatibility {
  override def run(args: Seq[String]): Unit = {
    ...
    PlugGatherInit.init //初始化获取所有集合
    SparkFunctionInit.init
...
class AutoSuggestController extends CustomController {
  override def run(params: Map[String, String]): String = {
    ...
    val size = params.getOrElse("size", "50").toInt
