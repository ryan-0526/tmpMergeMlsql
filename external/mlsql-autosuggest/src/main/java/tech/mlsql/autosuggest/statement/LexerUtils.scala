...
object LexerUtils {
  ...
  def filterPrefixIfNeeded(candidates: List[SuggestItem], tokens: List[Token], tokenPos: TokenPos):List[SuggestItem] = {
    if (tokenPos.offsetInToken != 0) {
      ...
      if (tableName.size >= 3 && ("`${".equals(tableName.substring(0,3)) || "\"${".equals(tableName.substring(0,3)))) {
        if (tableName.size > 3 && (!"}".equals(tableName.substring(3,4)) && !"`".equals(tableName.substring(3,4))) &&
        !"\"".equals(tableName.substring(3,4))) {
          //candidates.filter(s => s.name.startsWith(tokens(tokenPos.pos).getText.substring(3, 4)))
          candidates.filter(s => StringUtils.startsWithIgnoreCase(s.name,tokens(tokenPos.pos).getText.substring(3, 4)))

        } else candidates
      } else {

        //candidates.filter(s => s.name.startsWith(tokens(tokenPos.pos).getText.substring(0, tokenPos.offsetInToken)))
        candidates.filter(s => StringUtils.startsWithIgnoreCase(s.name,tokens(tokenPos.pos).getText.substring(0, tokenPos.offsetInToken)))
      }
    } else candidates
  }

  //  双``内提示
  def backQuoteNeeded(candidates: List[SuggestItem], tokens: List[Token], tokenPos: TokenPos):List[SuggestItem] = {
    if (tokenPos.offsetInToken-1 != 0) {
      val tableName: String = tokens(tokenPos.pos).getText
      val head = tableName.head.toString
      val last = tableName.last.toString
      //判断用户输入字符的长度，如果长度符合，判断是否符合使用set变量的前缀
      if (tableName.size >= 3 && (head.equals("`") && last.equals("`"))) {
        if (tableName.size >= 4 && ("`${").equals(tableName.substring(0,3))) {
          //表示是set变量
          //candidates.filter(s => s.name.startsWith(tokens(tokenPos.pos).getText.substring(3, tokenPos.offsetInToken)))
          //不区分大小写查找
          candidates.filter(s => StringUtils.startsWithIgnoreCase(s.name,tokens(tokenPos.pos).getText.substring(3, tokenPos.offsetInToken)))
        } else {
          //不是set
          //candidates.filter(s => s.name.startsWith(tokens(tokenPos.pos).getText.substring(1, tokenPos.offsetInToken)))
          candidates.filter(s => StringUtils.startsWithIgnoreCase(s.name,tokens(tokenPos.pos).getText.substring(1, tokenPos.offsetInToken)))
        }
      } else {
        candidates.filter(s => s.name.startsWith(tokens(tokenPos.pos).getText.substring(0, tokenPos.offsetInToken)))
  ...
  def tableTokenPrefix(tokens: List[Token], tokenPos: TokenPos): String = {
    ...
    if (tokenPos.pos > 1 && tokens(tokenPos.pos - 1).getType == TokenTypeWrapper.DOT && !tokens(tokenPos.pos - 1).getText.equals("and")) {
