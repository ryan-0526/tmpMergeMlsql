...
class SQLDownloadExt(override val uid: String) extends SQLAlg with Logging with WowLog with WowParams {
  ...
  override def train(df: DataFrame, path: String, params: Map[String, String]): DataFrame = {
    ...
    val stream = ReflectHelper.field(response, "response").asInstanceOf[HttpResponse].getEntity.getContent    //获取response中的流
    val tarIS = new TarArchiveInputStream(stream)   //解压流
      ...
      while (entry != null) {
        if (tarIS.canReadEntryData(entry)) {
          if (!entry.isDirectory) {
            val dir = entry.getName.split("/").filterNot(f => f.isEmpty).dropRight(1).mkString("/")   // dir = ""
            downloadResultRes += DownloadResult(PathFun(originalTo).add(dir).add(entry.getName.split("/").last).toPath)
            logInfo(format(s"extracting ${downloadResultRes.last.hdfsPath}"))   // /tmp/upload/文件名
            HDFSOperator.saveStream($(to) + "/" + dir, entry.getName.split("/").last, tarIS)    // $(to) = /home/users/ryan@gmail.com/tmp/upload
          }
