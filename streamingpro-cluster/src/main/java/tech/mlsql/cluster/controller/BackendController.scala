...
import tech.mlsql.cluster.model.{Backend, TestDemo}
...
class BackendController extends ApplicationController {
  ...
  @At(path = Array("/api_v1/test/getid"), types = Array(Method.GET))
  def testD = {
    val demo: TestDemo = TestDemo.findId(param("id").toInt)
    println(demo.getName)
    render(200, demo.getName)
  }

  @At(path = Array("/api_v1/test/getall"), types = Array(Method.GET))
  def testGetAll = {
    val users: util.List[TestDemo] = TestDemo.items()
    render(200,users)
  }
