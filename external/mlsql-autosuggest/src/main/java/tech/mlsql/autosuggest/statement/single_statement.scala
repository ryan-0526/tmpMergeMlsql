...
class SingleStatementAST(val selectSuggester: SelectSuggester, var start: Int, var stop: Int, var parent: SingleStatementAST) {
  ...
  def tables(tokens: List[Token]) = {
    ...
    //如果子查询是空则 创建NoneToken 不是返回token
    ...
    // T__3 == .
    // extract from `from`      //如果有子查询获取子查询中的from后的表
    ...
    val tempTable1 = fromTables.iterate(fromStart, tokensWithoutSubQuery.size)

    //如果想在子查询中使用主表，则需要获取
    val homeFromStart = TokenMatcher(tokens.slice(0, start), 0).asStart(Food(None, SqlBaseLexer.FROM), 1).start
    val tempTable2 = fromTables.iterate(homeFromStart, tokensWithoutSubQuery.size)

    val tempTables = (tempTable1 ++ tempTable2).distinct*/

    /**
      * 循环的目的是，为了获取多次from 后查询的表信息
      * 其二是为了如果语句中有子查询，那么首先解析的就是子查询语句，如果想要在子查询中提示主查询的信息，那么必然查询不到，所以循环取出本条sql
      * 所有表
      */
    val fromTables = new TableExtractor(selectSuggester.context, this, tokensWithoutSubQuery)
    var fromFlag = true
    var fromNum = 0
    var tempFromTables:List[MetaTableKeyWrapper] = List()
    while (fromFlag) {
      //val fromStart = TokenMatcher(tokensWithoutSubQuery.slice(fromNum, stop), 0).asStart(Food(None, SqlBaseLexer.FROM), offset = 1).start
      //val fromStart = TokenMatcher(tokensWithoutSubQuery.slice(fromNum, tokensWithoutSubQuery.size), 0).asStart(Food(None, SqlBaseLexer.FROM), offset = 1).start
      val fromStart = TokenMatcher(tokens.slice(fromNum, tokensWithoutSubQuery.size), 0).asStart(Food(None, SqlBaseLexer.FROM), offset = 1).start
      if (fromStart > 0){
        fromNum += fromStart
        tempFromTables = tempFromTables ++ fromTables.iterate(fromNum, tokens.size)
      } else {
        fromFlag = false
      }
    ...
    var flag = true
    var num = 0
    var tempJoinTables:List[MetaTableKeyWrapper] = List()
    while (flag) {
      //val joinStart = TokenMatcher(tokensWithoutSubQuery.slice(num, stop), start).asStart(Food(None, SqlBaseLexer.JOIN), offset = 1).start
      val joinStart = TokenMatcher(tokensWithoutSubQuery.slice(num, tokensWithoutSubQuery.size), 0).asStart(Food(None, SqlBaseLexer.JOIN), offset = 1).start
      if (joinStart > 0){
        num += joinStart
        tempJoinTables = tempJoinTables ++ joinTables.iterate(num, tokens.size)
      } else {
        flag = false
      }
    }
    ...
    tempFromTables ++ tempJoinTables ++ subqueryTables
  }
  ...
  def output(tokens: List[Token]): List[String] = { //截取列名
    ...
    extractor.iterate(selectStart, tokens.size)   //返回列名集合
  }
...
object SingleStatementAST {
  ...
  def _build(selectSuggester: SelectSuggester, tokens: List[Token], start: Int, stop: Int, isSub: Boolean = false): SingleStatementAST = {
    ...
    for (index <- (start until stop) if index >= jumpIndex) {
      val token = tokens(index)   //判断是否是子查询sql, 如果是先解析子查询中的表和列信息
