...
import tech.mlsql.autosuggest.statement.{SelectSuggester, _}
...
class AutoSuggestContext(val session: SparkSession,
  ...
  private[autosuggest] def _suggest(tokenPos: TokenPos): List[SuggestItem] = {
    ...
    val items = _statements(index).headOption.map(_.getText) match { //匹配语句头关键字
      ...
      case Some("save") =>
        val suggester: SaveSuggester = new SaveSuggester(this, _statements(index), relativeTokenPos)
        suggester.suggest()

      case Some("register") =>
        val suggester: RegisterSuggester = new RegisterSuggester(this, _statements(index), relativeTokenPos)
        suggester.suggest()

      case Some("run") =>
        val suggester: RunTrainSuggester = new RunTrainSuggester(this, _statements(index), relativeTokenPos)
        suggester.suggest()
  ...
  private val firstWords = List("load", "select", "include", "register", "run", "train", "save", "set",
    "set hello = world where type =\"text\";",
    "set codeBlock = '''\n\n''';",
    "run testData as RepartitionExt.`` where partitionNum=\"5\" as newdata;",
    "register ScriptUDF.`` as registerTable;",
    "save append tempTable as jdbc.``;").map(SuggestItem(_, SpecialTableConst.KEY_WORD_TABLE, Map())).toList
}
