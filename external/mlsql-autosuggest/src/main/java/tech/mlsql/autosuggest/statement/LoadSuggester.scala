...
class LoadSuggester(val context: AutoSuggestContext, val _tokens: List[Token], val _tokenPos: TokenPos) extends StatementSuggester
  ...
  register(classOf[LoadAsSuggester])
...
//解决load语句中as不提示问题
class LoadAsSuggester(loadSuggester: LoadSuggester) extends StatementSuggester with StatementUtils{
  ...
  def context = loadSuggester.context
