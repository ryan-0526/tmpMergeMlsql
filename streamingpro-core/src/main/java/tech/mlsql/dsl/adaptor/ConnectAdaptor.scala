...
import streaming.dsl.{ConnectMeta, DBMappingKey, ScriptSQLExec, ScriptSQLExecListener}
...
class ConnectAdaptor(scriptSQLExecListener: ScriptSQLExecListener) extends DslAdaptor {
  ...
  override def parse(ctx: SqlContext): Unit = {
    ...
    val skipConnect = context.userDefinedParam.getOrElse("__skipConnect__", "__skipConnect__")
    if (!skipConnect.toLowerCase.equals("true")) {
      throw new MLSQLException("connect Refuse to enforce")
    }
