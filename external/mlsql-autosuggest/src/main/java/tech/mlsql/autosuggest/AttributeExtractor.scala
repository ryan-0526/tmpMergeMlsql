...
class AttributeExtractor(autoSuggestContext: AutoSuggestContext, ast: SingleStatementAST, tokens: List[Token]) extends MatchAndExtractor[String] {
  ...
  //修改地方一：查询语句中不加as,也能提示列名
  private def notAsName(start: Int): TokenMatcher = {
    /*val temp = TokenMatcher(tokens, start).
      eat(Food(None, SqlBaseLexer.INTEGER_VALUE), Food(None, SqlBaseLexer.IDENTIFIER)).
      build*/
    val temp = TokenMatcher(tokens, start).
      eatOneAny.
  ...
  private def attributeM(start: Int): TokenMatcher = { //判断是否是属性
    val temp = TokenMatcher(tokens, start).
      eat(Food(None, SqlBaseLexer.IDENTIFIER), Food(None, SqlBaseLexer.T__3)).optional.
      eat(Food(None, SqlBaseLexer.IDENTIFIER)). //一会加入eat(Food(None, SqlBaseLexer.INTEGER_VALUE), Food(None, SqlBaseLexer.IDENTIFIER))
  ...
  override def extractor(start: Int, end: Int): List[String] = {
    ...
    if (token.getType == SqlBaseLexer.ASTERISK) { //如果列中出现 '*' 则将查询表中的属性存入集合
