...
trait SQLAlg extends Serializable {
  ...
  /*def pluParam () : mutable.Seq[Param[String]] ={
    mutable.Seq[Param[String]]()
  }*/
...
/*object SQLAlg {
  ...
  //存储(插件名 -> 插件类中参数
  private val plugParams = new mutable.HashMap[String, mutable.Seq[Param[String]]]()

  def getPlug(name:String) = {
    val sqlAlg = MLMapping.findAlg(name)
    plugParams += (name -> sqlAlg.pluParam())
  }

  private val mapping = MLMapping.mapping
  ...
  /*mapping.map{
    case (key,value) =>
      val value1: SQLAlg = Class.forName(value).newInstance().asInstanceOf[SQLAlg]
      plugParams += (key -> value1.pluParam())
  }*/
}*/
...
case object BaseType extends ModelType("baseType", "base")
