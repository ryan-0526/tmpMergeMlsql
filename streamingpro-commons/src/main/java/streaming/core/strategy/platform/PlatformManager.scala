...
class PlatformManager extends Logging {
  ...
  def run(_params: ParamsUtil, reRun: Boolean = false) = {
    ...
    //这里的asScala是将Java中的map集合转换成scala中的可变的map集合(scala.collection.mutable.Map包下的)，因为他要操作配置，将配置重新添加到一个新的map当中
...
object PlatformManager {
  ...
  //通过反射创建SparkRuntime对象，并且找到getOrcreate方法，传入配置参数，最后转为StreamingRuntime
